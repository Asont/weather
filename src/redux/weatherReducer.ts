...
export type StateType = {
    ...
    //   descriptions: string[]
    backgroundsAndTypesOfWeather: BackgroundsAndDiscriptionsType[]
}
...
export type ActionTypeDefaultAPI =
    | DefaultWeatherStateACType
    | DefaultCityAndCountryACType

export const weatherReducer = (
    state: StateType = initialState,
    action: ActionTypeDefaultAPI
) => {
    switch (action.type) {
        case 'SET_WEATHER': {
            return {
                ...
                temperatureFuture: [
                    {
                        ...
                        descriptor: action.payload.daily[0].weather[0].main,
                    },
                    {
                        temp: action.payload.daily[1].temp.day,
                        descriptor: action.payload.daily[1].weather[0].main,
                    },
                    {
                        temp: action.payload.daily[2].temp.day,
                        descriptor: action.payload.daily[2].weather[0].main,
                    },
                    {
                        temp: action.payload.daily[3].temp.day,
                        descriptor: action.payload.daily[3].weather[0].main,
                    },
                    {
                        temp: action.payload.daily[4].temp.day,
                        descriptor: action.payload.daily[4].weather[0].main,
                    },
                    {
                        temp: action.payload.daily[5].temp.day,
                        descriptor: action.payload.daily[5].weather[0].main,
                    },
                ],
            }
        }
...
export const weatherTC =
    (payload: WeatherPayload) => (dispatch: TypedDispatch) => {
        if (payload.cache && Object.keys(payload.cache).length === 0) {
            ...
            if (countryAndCityName != null) {
                ...
                dispatch(
                    CityAndCountryAC(
                        countryAndCityNameInObject.city,
                        countryAndCityNameInObject.country
                    )
                )
            }
            dispatch(loadingSetAC(false))
...
export const searchWeatherTC =
    (args: { city: string; api: WeatherApi }) => (dispatch: TypedDispatch) => {
        weatherApi
            .searchLAtAndLonForCity(args.city)
            .then((res) => {
                dispatch(
