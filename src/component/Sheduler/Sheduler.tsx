...
const Sheduler = () => {
    ...
    const data = useSelector<RootReducerType, Array<DataType>>(state => state.sheduler.shedulerData)

    const newData = data.map((item,index) => {
        return (
            <div key={index}><EditableInputTime time={item.time}/><EditableInputText text={item.text}/></div>
        )
    })
